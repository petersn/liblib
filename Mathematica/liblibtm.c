/*
 * This file automatically produced by /home/peter/.cells/mathematica/mathematica/SystemFiles/Links/MathLink/DeveloperKit/Linux-x86-64/CompilerAdditions/mprep from:
 *	liblib.tm
...
int load_library_C P(( const char * _tp1));
...
int call_lib_function P(( int _tp1, const char * _tp2, int _tp3));
...
#endif
{
	...
	_tp0 = call_lib_function(_tp1, _tp2, _tp3);
...
static struct func {
	...
	char  *f_name;
		...
		{ 1, 0, _tr0, "load_library_C" },
		{ 3, 0, _tr1, "call_lib_function" }
		};

static char* evalstrs[] = {
	"LoadLibraryC::usage = \"LoadLibraryC[path] loads the shared objec",
	"t at path, or searches for the system SO path\"",
	(char*)0,
	"LoadLibraryC::usage = \"CallLibFunction[lib, function, arg] calls",
	" a function from a loaded library\"",
	(char*)0,
	(char*)0
};
...
#endif
{
	int _res;
	_res = MLConnect(mlp);
	if (_res) _res = _definepattern(mlp, "LoadLibraryC[s_String]", "{s}", 0);
	if (_res) _res = _doevalstr( mlp, 0);
	if (_res) _res = _definepattern(mlp, "CallLibFunction[index_Integer, function_String, arg_Integer]", "{index, function, arg}", 1);
